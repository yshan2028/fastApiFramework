...
def creat_customize_log_loguru(pro_path=None):
    ...
    logger.add(err_log_file_path, format=format_err, rotation='00:00', encoding='utf-8', level='ERROR',
    ...
    # enqueue=True表示 开启异步写入
    # 使用 rotation 参数实现定时创建 log 文件,可以实现每天 0 点新创建一个 log 文件输出了
    logger.add(log_file_path, format=format_info, rotation='00:00', compression="zip", encoding='utf-8', level='INFO',
               enqueue=True)  # Automatically rotate too big file
